...
// NewSampleDistributionFromRandDist si similar to NewSampleDistributionFromRand
...
// S_0=InitState(). It is assumed that generating multiple Y sequences from S_0
// is statistically equivalent to generating a single consecutive sequence. When
// this property does not hold, set BatchMin in config to a larger value than
// the required number of Y's, so that all the values are generated by the same
// job sequentially.
//
// Note, however, that Transform generates a Markow chain, which in general may
// not be appropriate to model by a random variable with any particular
// distribution. Use a stateful generation with care.
...
func (d *RandDistribution) Histogram() *Histogram {
	...
	if d.histogram == nil {
		...
		m := parallel.Map(d.context, d.config.Workers, d.jobsIter(d.config.Workers))
		for {
			v, err := m.Next()
...
// single sequence of source samples. This reduces the number generated source
// samples from N*numSamples to N+numSamples.  In practice, multiple such
